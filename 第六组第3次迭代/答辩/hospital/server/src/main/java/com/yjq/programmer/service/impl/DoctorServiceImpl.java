...
public class DoctorServiceImpl implements IDoctorService {
    ...
    public ResponseDTO<PageDTO<DoctorDTO>> getDoctorList(PageDTO<DoctorDTO> pageDTO, String token) {
        ...
        if(RoleEnum.ADMIN.getCode().equals(roleId) || RoleEnum.USER.getCode().equals(roleId)){
            // 如果是管理员角色，可以看到所有数据
            // 判断是否进行关键字搜索
//            if(!CommonUtil.isEmpty(pageDTO.getSearchContent())) {
//                doctorExample.createCriteria().andNameLike("%" + pageDTO.getSearchContent() + "%");
//            }
            if(!CommonUtil.isEmpty(pageDTO.getOfficeId())) {
                doctorExample.createCriteria().andNameLike("%" + pageDTO.getSearchContent() + "%").andOfficeIdEqualTo(pageDTO.getOfficeId());
            }
            else {
                doctorExample.createCriteria().andNameLike("%" + pageDTO.getSearchContent() + "%");
            }
            ...
            // 如果是医生角色，只能看到自己数据
            // 判断是否进行关键字搜索
            if(!CommonUtil.isEmpty(pageDTO.getOfficeId()/*.getSearchContent()*/)) {
                doctorExample.createCriteria().andNameLike("%" + pageDTO.getSearchContent() + "%").andIdEqualTo(id).andOfficeIdEqualTo(pageDTO.getOfficeId());
            }else{
                doctorExample.createCriteria().andIdEqualTo(id).andNameLike("%" + pageDTO.getSearchContent() + "%");
            }
        }
        // 不知道当前页多少，默认为第一页
